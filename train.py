...
def fit_one_epoch(net,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    with tqdm(total=epoch_size,desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(gen):
            ...
            with torch.no_grad():
                ...
                pngs = torch.from_numpy(pngs).type(torch.FloatTensor).long()  # png图像
            ...
            optimizer.zero_grad()
            outputs = net(imgs)
            loss    = CE_Loss(outputs, pngs, num_classes=NUM_CLASSES)  # 交叉熵损失函数
            if dice_loss:
            ...
            with torch.no_grad():
                ...
                #   计算f_score
                #-------------------------------#
                _f_score = f_score(outputs, labels)  # 计算F1
    ...
    loss_history.append_loss(total_loss/(epoch_size+1), val_loss.cpu()/(epoch_size_val+1))
...
if __name__ == "__main__":
    ...
    #---------------------------------------------------------------------# 
    dice_loss = True  # 该loss是可以解决样本不平衡问题，但容易造成梯度爆炸
    #-------------------------------#
    #   主干网络预训练权重的使用
    #-------------------------------#
    pretrained = True
